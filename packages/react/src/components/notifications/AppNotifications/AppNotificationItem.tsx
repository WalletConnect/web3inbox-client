...
const AppNotificationItem: React.FC<IAppNotificationProps> = ({
  ...
  const [notificationsDrag, setNotificationsDrag] =
    useContext<AppNotificationsDragContext>(AppNotificationDragContext);
  ...
  const handleClearClick = useCallback(() => {
    dragControls.start("hidden");
    deleteMessage(Number(notification.id)).then(onClear);
    actionControls.start("hidden");
  }, [deleteMessage, onClear]);
  ...
  const handleDragEnd = (_: never, info: PanInfo) => {
    const shouldShowControls =
      info.velocity.x >= 50 || info.offset.x > DRAG_OFFSET / 2;
  ...
  const handleDrag = (_: never, info: PanInfo) => {
    const shouldShowActions =
      info.velocity.x >= 0 || info.offset.x > DRAG_OFFSET / 2;
      ...
      >
        ...
        >
          ...
          <button
            ...
            className="AppNotificationsActions__unread"
          >
            <img src={UnreadIcon} />
        ...
        <div key={notification.id} className="AppNotifications__item__content">
          <div className="AppNotifications__item__header">
            ...
            <div className="AppNotifications__item__header__wrapper">
              {formattedTime ? (
                <Text variant="tiny-500">{formattedTime}</Text>
              ) : null}
          ...
          {textClamped && (
            <p className="AppNotifications__item__show">
              {show ? "Show less" : "Show more"}
            </p>
          )}
          {notification.url && (
            <ExternalLink link={notification.url}>Visit Link</ExternalLink>
          )}
        </div>
      </m.div>
    </LazyMotion>
