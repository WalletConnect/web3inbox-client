...
export const useSearch = () => {
  ...
  useEffect(() => {
    const chatSearchSubscription = chatSearchService.searchState.subscribe(
      (isOpen) => {
        setIsChatSearchOpen(isOpen);
      }
    );
    const pushSearchSubscription = pushSearchService.searchState.subscribe(
      (isOpen) => {
        setIsPushSearchOpen(isOpen);
      }
    );
    const appSearchSubscription = appSearchService.searchState.subscribe(
      (state) => {
...
export const useModals = () => {
  ...
  useEffect(() => {
    const profileSubscription = profileModalService.modalState.subscribe(
      (isOpen) => {
    ...
    const contactsSubscription = contactsModalService.modalState.subscribe(
      (isOpen) => {
        setIsContactModalOpen(isOpen);
      }
    );
    const shareSubscription = shareModalService.modalState.subscribe(
      (isOpen) => {
        setIsShareModalOpen(isOpen);
      }
    );
    const preferencesSubscription =
      preferencesModalService.modalState.subscribe((state) => {
        setPreferencesModalAppId(state.preferencesModalAppId);
        setIsPreferencesModalOpen(state.isOpen);
      });
    const unsubscribeSubscription =
      unsubscribeModalService.modalState.subscribe((state) => {
